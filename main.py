...
# Token verification via link shortener (set TOKEN_VERIFY=true in .env to enable)
TOKEN_VERIFY_ENABLED = os.environ.get("TOKEN_VERIFY", "false").lower() == "true"
TOKEN_TTL_SECONDS    = int(os.environ.get("TOKEN_TTL", "600"))   # default 10 min
...
async def start_handler(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    # ── Verified file deep-link (after token completion) ─────────────────────
    if args and args[0].startswith("verified_"):
        unique_id = args[0][len("verified_"):]
        ...
        logger.info("[DELIVERY][VERIFIED] unique_id=%s | user_id=%s", unique_id, user_id)
        await update.message.reply_text(
    ...
    if args and args[0].startswith("file_"):
        ...
        # 3. Token verification (if enabled)
        if TOKEN_VERIFY_ENABLED:
            # Generate a one-time token and verification link
            token    = secrets.token_urlsafe(24)
            save_token(unique_id, token, ttl_seconds=TOKEN_TTL_SECONDS)

            verify_url  = f"{WEBHOOK_URL}/verify/{unique_id}/{token}"
            short_url   = shorten_url(verify_url)   # shorten via GPlink/VPlink etc.
            minutes     = TOKEN_TTL_SECONDS // 60

            logger.info(
                "[TOKEN] Verification required | unique_id=%s | user_id=%s | short_url=%s",
                unique_id, user_id, short_url,
            )

            await update.message.reply_text(
                f"🔐 *Verification Required*\n"
                f"━━━━━━━━━━━━━━━━━━━━\n\n"
                f"To access this file, please complete a quick verification step.\n\n"
                f"👇 *Click the link below, complete it, then come back:*\n"
                f"{short_url}\n\n"
                f"⏱ Link expires in *{minutes} minutes*.\n\n"
                f"━━━━━━━━━━━━━━━━━━━━\n"
                f"_After completing verification, the file will be sent automatically._",
                parse_mode="Markdown",
                disable_web_page_preview=True,
            )
            return

        # 4. Deliver file (no token required)
...
@flask_app.get("/verify/<unique_id>/<token>")
def verify_token(unique_id: str, token: str) -> Response:
    """
    Token verification endpoint: /verify/<unique_id>/<token>

    Flow:
      1. Validate token in MongoDB (not used, not expired)
      2. If valid  → mark used, send file to user via bot, show success page
      3. If invalid → show error page

    Note: This endpoint is what the shortener URL redirects to.
    The user opens the shortener, completes the ad/captcha, lands here.
    We then push the file directly to their Telegram chat.
    """
    ...
    logger.info(
        "[VERIFY] Token check | unique_id=%s | token=%s... | ip=%s",
        unique_id, token[:8], ip,
    )

    # ── Validate token ────────────────────────────────────────────────────────
    is_valid = verify_and_consume_token(unique_id, token)

    if not is_valid:
        logger.warning("[VERIFY] Invalid/expired token | unique_id=%s | ip=%s", unique_id, ip)
        html = _render_verify_page(
            success=False,
            unique_id=unique_id,
    ...
    # ── Valid: redirect user back to bot with verified flag ───────────────────
    tg_link = f"https://t.me/{BOT_USERNAME}?start=verified_{unique_id}"
    logger.info("[VERIFY] Token valid — redirecting to bot | unique_id=%s", unique_id)
...
def _render_verify_page(
    ...
    """Render HTML page shown after token verification attempt."""

    if success:
        icon    = "✅"
        heading = "Verification Complete!"
        message = "You can now receive your file. Tap the button below to open Telegram."
        btn_cls = "btn-success"
        btn_txt = "Get My File →"
        btn_href = tg_link
        ...
        message  = "This link has expired or has already been used. Please request a new file link."
        btn_cls  = "btn-error"
        btn_txt  = "Go to Bot"
...
<head>
  ...
  <style>
    * {{ margin: 0; padding: 0; box-sizing: border-box; }}
    ...
    h2       {{ font-size: 22px; margin-bottom: 12px; color: #ffffff; }}
